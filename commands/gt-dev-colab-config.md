...
Read the config file at `~/.geno-tools/geno-dev/configs/colab.json`. If it doesn't exist, create it with defaults:
...
- Write the updated config to `~/.geno-tools/geno-dev/configs/colab.json`
- Display the final configuration

### 6. Update README if needed

If `~/.geno-tools/geno-dev/configs/` is newly created, ensure the `~/.geno-tools/geno-dev/README.md` structure section includes the `colab/` directory.
